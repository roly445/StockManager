...
namespace StockManager.Domain.Tests.CommandValidators.ApplicationUser
{
    public class CreateUserCommandValidatorTests
    {
        ...
        public void GivenNormalizedUserNameIsEmpty_ExpectFail()
        {
            ...
            var command = new CreateUserCommand("Emile-A239", string.Empty, "some-hash");
        ...
        {
            var validator = new CreateUserCommandValidator();
            var command = new CreateUserCommand("Emile-A239", "emile-a239", string.Empty);
            var result = validator.Validate(command);
            Assert.False(result.IsValid);
        ...
        {
            var validator = new CreateUserCommandValidator();
            var command = new CreateUserCommand(string.Empty, "emile -a239", "some-hash");
            var result = validator.Validate(command);
            Assert.False(result.IsValid);
